...
class PolyTimeLSTM(pufferlib.models.LSTMWrapper):
    def __init__(self, env, policy, input_size = 256, hidden_size = 256):
        super().__init__(env, policy, input_size, hidden_size)

class PolyTime(nn.Module):
    def __init__(self, env, hidden_size = 256, **kwargs):
        super().__init__()
 
        self.hidden_size = hidden_size
        self.max_agents = env.max_agents
        self.max_items = env.max_items
        self.max_utility = env.max_utility
        ...
        self.encoder= nn.Sequential(
            pufferlib.pytorch.layer_init(nn.Linear(self.num_obs + self.max_agents + self.max_items, hidden_size), std=0.01),
            nn.ReLU(),
        )
    ...
    def forward(self, observations, state=None) -> Tuple[torch.Tensor, torch.Tensor]:
        hidden = self.encode_observations(observations)
        probs, value = self.decode_actions(hidden)
        return probs, value

    def forward_train(self, observations: torch.Tensor, state=None) -> Tuple[torch.Tensor, torch.Tensor]:
        hidden = self.encode_observations(observations)
        probs, value = self.decode_actions(hidden)
        return probs, value

    def encode_observations(self, observations: torch.Tensor, state=None) -> torch.Tensor:
        # if (observations[:, 0] < 0).any():
        #     print("observations -ve")
        # if (observations[:, 0] > self.max_agents).any():
        #     print(observations[:, 0])
        #     print("observations agent id out of range")
        # if (observations[:, 1] < 0).any():
        #     print("observations item id out of range")
        # if (observations[:, 1] > self.max_items).any():
        #     print("observations item id out of range")
        
        
        # assert (observations[:, 0] >= 0).all() and (observations[:, 0] <= self.max_agents).all(), \
        #      f"agent id out of range 0‥{self.max_agents-1}"
        # assert (observations[:, 1] >= 0).all() and (observations[:, 1] <= self.max_items).all(), \
        #      f"item id out of range 0‥{self.max_items-1}"
             
             
        oh1 = F.one_hot(observations[:, 0].long(), num_classes=self.max_agents+1).float()
        oh2 = F.one_hot(observations[:, 1].long(), num_classes=self.max_items+1).float()

        ut = observations[:, 2:2+self.max_agents*self.max_items].float() / self.max_utility

        x = torch.cat([oh1, oh2, ut, observations[:, 2+self.max_agents*self.max_items:]], dim=-1)
        # print("x shape: ", x.shape)

        # print("target shape:", self.num_obs + self.max_agents + self.max_items)
    ...
    def decode_actions(self, hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        action_std = action_std.clamp(min=1e-6)  # Avoid zero std
        action_std=  action_std.clamp(max=5.0)  # Avoid too large std
